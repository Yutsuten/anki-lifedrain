...
class Settings(object):
    ...
    def preferences_ui(self, form):
        ...
        row = 0
        self._create_label(form, row, '<b>Bar behaviour</b>')
        row += 1
        self._create_check_box(form, row, 'stopOnAnswer', 'Stop drain on answer shown')
        row += 1
        self._create_check_box(form, row, 'disableAddon', 'Disable Life Drain (!)')
        row += 1
        self._create_label(form, row, '<b>Bar style</b>')
        row += 1
        self._create_combo_box(form, row, 'positionList', 'Position', POSITION_OPTIONS)
        row += 1
        self._create_spin_box(form, row, 'heightInput', 'Height', [1, 40])
        row += 1
        self._create_spin_box(form, row, 'borderRadiusInput', 'Border radius', [0, 20])
        row += 1
        self._create_combo_box(form, row, 'textList', 'Text', TEXT_OPTIONS)
        row += 1
        self._create_combo_box(form, row, 'styleList', 'Style', STYLE_OPTIONS)
        row += 1
        self._create_color_select(form, row, 'bgColor', 'Background color')
        row += 1
        self._create_color_select(form, row, 'fgColor', 'Foreground color')
        row += 1
        self._create_color_select(form, row, 'textColor', 'Text color')
        row += 1
        self._fill_remaining_space(form, row)
    ...
    def deck_settings_ui(self, form):
        ...
        row += 1
        self._create_spin_box(form, row, 'maxLifeInput', 'Maximum life', [1, 10000])
        row += 1
        self._create_spin_box(form, row, 'recoverInput', 'Recover', [0, 1000])
        row += 1
        self._create_check_box(form, row, 'enableDamageInput', 'Enable damage')
        row += 1
        self._create_spin_box(form, row, 'damageInput', 'Damage', [-1000, 1000])
        row += 1
        self._create_spin_box(form, row, 'currentValueInput', 'Current life', [0, 10000])
        row += 1
        self._fill_remaining_space(form, row)
        form.tabWidget.addTab(form.lifedrain_widget, 'Life Drain')
    ...
    def _create_label(self, form, row, text, color=None):
        '''
    ...
    def _create_combo_box(self, form, row, cb_name, label_text, options):
        '''
    ...
    def _create_check_box(self, form, row, cb_name, label_text):
        '''
    ...
    def _create_spin_box(self, form, row, sb_name, label_text, val_range):
        '''
    ...
    def _create_color_select(self, form, row, cs_name, label_text):
        '''
    ...
    def _fill_remaining_space(self, form, row):
        '''
